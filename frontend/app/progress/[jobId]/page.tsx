...
export default function ProgressPage({ params }: ProgressPageProps) {
  ...
  const [glbUrl] = useState<string>(() => {
    // Generate a placeholder URL - in real implementation, backend would return this
    return `/api/models/${modelId}/geometry.glb`;
  });
        ...
        <div className="text-center space-y-4">
          {!showViewer ? (
            <>
              ...
              <p className="text-charcoal-500">Proszę czekać, może to chwilę potrwać</p>
            </>
          ) : (
            <>
              <h1 className="text-4xl font-bold text-black-DEFAULT">
              ...
              <p className="text-charcoal-500">Twój model jest gotowy do przeglądania</p>
            </>
          )}
        </div>
        ...
        {!showViewer && !isFailed && (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-3">
              ...
              <Progress
                value={percentage}
                className="h-3 rounded-full bg-bright_snow-600 border border-lilac_ash-200"
              />
            </div>
